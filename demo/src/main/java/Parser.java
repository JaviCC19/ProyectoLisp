...
public class Parser {
    ...
    private Node parseExpression() {
        ...
        if ("(".equals(token)) { // Si el token es un paréntesis de apertura
            List<Node> children = new ArrayList<>(); // Crear una lista para almacenar los nodos hijos
            while (!")".equals(tokens.get(currentPosition))) { // Mientras no se encuentre un paréntesis de cierre
                Node child = parseExpression(); // Analizar la expresión recursivamente para obtener el nodo hijo
            ...
            return new ListNode(children); // Devolver un nodo de lista con los nodos hijos
            ...
            return new AtomNode(token); // Crear un nodo átomo con el token como valor
        } else {
